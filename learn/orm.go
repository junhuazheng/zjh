...
//根据实体通过反射获取表信息
//返回表信息
func getTableInfo(model interface{})(tableInfo *TableInfo, err error) {
	defer func() {
	...
	err = nil 
	tableInfo = &TableInfo{}
	tableInfo.TMMap = make(map[string]string)
	...
	for i, j := 0, rt.NumField(); i < j; i++ {
		...
		//没有tag，表字段与实体字段ing一致
		if rtf.Tag == "" {
			f = FieldInfo{Name:rtf.Name, IsAutoGenerate:false, IsPrimaryKey:false, Value:rvf}
			tableInfo.TMMap[rtf.Name] = rtf.Name
		} else {
			...
			if strings.Index(strTag, ":") == -1 {
				...
				tableInfo.TMMap[strName] = rtf.Name
			}
		}
...
func GenerateDeleteSql(model interface{})(string, []interface{}, error) {
	...
	//根据字段信息拼Sql语句，以及参数值
	strSql := "delete form " + tbInfo.Name + " where "
	var idVal interface{}
	for _, v := range tbInfo.Fields {
		if v.IsPrimaryKey {
			strSql += v.Name + "=?"
